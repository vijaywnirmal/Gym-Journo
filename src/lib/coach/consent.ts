...
//
// `recipient` names who receives the data (see providerLabel in ai.ts) — the wording must always
// match the provider that is actually configured.
export function coachConsent(recipient: string) {
  return {
    summary: `Coach explains what your training records show. To do that, your training data is sent to ${recipient} when you ask a question.`,
    shared: [
